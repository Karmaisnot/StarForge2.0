...
const EMPLOYEES = [
  { n: 'Nigora Karimova', pos: 'Katta o‘qituvchi', dept: 'Matematika', b: 'Yunusobod', type: 'full', sal: 8400000, since: '2021', st: 'active' },
  { n: 'Aziz Tursunov', pos: 'O‘qituvchi', dept: 'Ingliz tili', b: 'Chilonzor', type: 'full', sal: 7800000, since: '2022', st: 'active' },
  { n: 'Sevara Olimova', pos: 'Assistent', dept: 'Matematika', b: 'Yunusobod', type: 'half', sal: 4200000, since: '2024', st: 'active' },
  { n: 'Gulnora Saidova', pos: 'Reception boshlig‘i', dept: 'Qabul', b: 'Mirobod', type: 'full', sal: 5600000, since: '2023', st: 'active' },
  { n: 'Jasur Rahimov', pos: 'O‘qituvchi', dept: 'Tabiiy fanlar', b: 'Sebzor', type: 'full', sal: 7000000, since: '2023', st: 'leave' },
  { n: 'Nodira Karimova', pos: 'SMM menejer', dept: 'Marketing', b: 'Yunusobod', type: 'hourly', sal: 3800000, since: '2025', st: 'active' },
];
...
export function HRPage({ role }) {
  ...
  const typeLabel = { full: t('settings.valFull'), half: t('status.partial'), hourly: t('cols.tenure') };
  ...
  return (
    <>
      ...
      <div className="ad-kpi-grid">
        <Kpi label={t('hr.kpiTotal')} value={ceo ? '82' : '16'} icon={Icons.user} trend={{ up: true, v: '4' }} />
        <Kpi label={t('hr.kpiVacancy')} value="7" accent="var(--sf-warn)" sub={t('hr.vacancySub')} icon={Icons.flag} />
        <Kpi label={t('hr.kpiFund')} money={ceo ? 512000000 : 96000000} accent="var(--sf-success)" />
        <Kpi label={t('hr.kpiTenure')} value="2.4 yil" />
        <Kpi label={t('hr.kpiOnLeave')} value="3" accent="var(--sf-primary)" />
      </div>
      <SectionHeader action={<a className="ad-link" onClick={() => a.open(t('hr.allCandidates'))}>{t('hr.allCandidates')} ›</a>}>{t('hr.recruiting')}</SectionHeader>
        ...
        {pipeline.map((col) => (
          <div key={col.id} className="ad-kcol">
            ...
            <div className="ad-kcards">
              {col.cands.map((c, i) => (
                <div key={i} className="og-cand" onClick={() => a.open(c.n)}>
      ...
      <SectionHeader action={<Segmented value={seg} onChange={setSeg} options={[{ id: 'all', label: t('hr.segAll') }, { id: 'teacher', label: t('hr.segTeacher') }, { id: 'admin', label: t('hr.segAdmin') }]} />}>{t('hr.staffSection')} · {ceo ? 82 : 16}</SectionHeader>
      <Card pad={false}>
        <DataTable cols={[
